...
| Reviewer | Type | Reported (H/M/L/I) | Valid & corroborated | Recall /18 | Severity drift | PoC quality | Signature hit | Worst blind spot |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| **Certora** | Automated pipeline | 0/3/10/24 | 3 M + 8 corroborations | **11** | reference calibration | Automated, reproducible | Swap-and-pop desync — sole finder | Zero Highs; rated the pause-OOG brake-kill only Medium |
...
| **Codex** | LLM | 0/4/–/– | 4/4 (3 corroborated, 1 unchallenged) | **4** | +1.5 | Concrete inline PoCs | Zero false positives; every finding corroborated or unchallenged | Narrowest non-empty surface (4 issues, no lifecycle/config depth) |

**Reading the table:** *Recall /18* = share of the 18 canonical issues hit (defined on Page 3). *Severity drift* = average notch inflation vs. the Certora rating where both reviewed the same issue. *Corroborated* = independently matched by Certora and/or C4; *unchallenged* = not contradicted by any other review.
...
Best raw recall (11/18) and the only reviewer to find the swap-and-pop revocation desync — a cross-transaction state bug every other reviewer missed. Its weakness is severity conservatism (zero Highs; rated the guardian-brake kill Medium) and no exploitation narratives. As a calibration reference it is invaluable: every LLM finding that survives Certora cross-checking is real.
...
Four findings (expiration overflow variant, SENTINEL owners, recovery-delay pre-elapse, failed Safe-module calls recorded as executed), every one corroborated or unchallenged, zero false positives, concrete inline PoCs. The trade-off is breadth: the smallest non-empty issue surface, no lifecycle or configuration depth.

---

# Page 3 — Accuracy Analysis

### Canonical issue set (18)

Union of all reviews after dedup: **13 model issues (F1–F13)** + **2 Certora-exclusive Mediums** (swap-and-pop desync; spell mutual non-exclusivity) + **3 C4 Mediums** (gas griefing; index `+1` bug; lowered-expiration revert). C4's 11 low/QA items and Certora's remaining L/I are excluded from recall math except where they corroborate a canonical issue.
...
| 18 | F13 failed Safe-module calls recorded as executed | · | ◐ L-06 | · | · | · | · | · | ✓ |

### Scoreboard

| Metric | Leader | Detail |
| --- | --- | --- |
| Raw recall | **Certora (11/18, 61%)** | GLM 5.3 & Claude tie for best LLM at 5/18 (28%); C4 itself only 3/18 exact (17%) |
...
2. **Codex** — perfect precision, but one finding short of the leaders' surface coverage.
